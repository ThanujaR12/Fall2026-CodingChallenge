...
- [X] T031 [US14] "For you": `GET /api/feed/for-you` (interests from saved tags weighted by recency plus board names, interleaved interest searches plus popular discoveries, saved photos excluded, per-visit seed); Home defaults to it with a "Picked for you" banner, interest chips, Shuffle, and a reason on each photo; tests; API.md
