...
- [ ] T117 Run quickstart scenarios S1–S8 in `specs/001-discover-collect/quickstart.md` against real MongoDB Atlas and a real Pixabay key in `server/.env` (not the local harness), time the S1 flow for SC-001, confirm search results appear in about 2 seconds, and fix any differences found per SC-001, SC-002, T114 (partial)
